...
def test_plan_routes_with_network_finds_path():
    ...
    with patch("routing.plan.get_assembled_snapshot", return_value=snap):
        req = RouteRequest(
            origin_lat=40.409, origin_lon=49.867, destination_lat=40.413, destination_lon=49.871
        )
        res = plan_routes(req)
...
def test_plan_routes_alternative_has_segments():
    req = RouteRequest(
        origin_lat=40.4093, origin_lon=49.8671, destination_lat=40.413, destination_lon=49.871
    )
    res = plan_routes(req)
