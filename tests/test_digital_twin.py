...
from iridium_schemas.network import NetworkNode, NetworkEdge
from digital_twin.service import get_graph, set_graph, update_state, get_snapshot
...
def test_get_assembled_snapshot_no_postgres():
    ...
    with patch.dict(os.environ, {}, clear=False):
        ...
        snap = get_assembled_snapshot(include_weather_provenance=False, include_traffic_provenance=False)
    assert snap.nodes == []
    assert snap.edges == []
    assert snap.data_status in ("configuration_required", "unavailable", None) or snap.source_provenance
...
def test_get_assembled_snapshot_weather_traffic_exception_branches():
    ...
    from digital_twin.state_assembler import get_assembled_snapshot
    with patch("digital_twin.state_assembler._load_network_from_env_or_db", return_value=([], [], "configuration_required", [])):
        snap = get_assembled_snapshot(include_weather_provenance=True, include_traffic_provenance=True)
