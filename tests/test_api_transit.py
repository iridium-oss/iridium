...
from iridium_schemas.transit import (
    TransitAgency, TransitRoute, TransitStop,
    Alert, SourceStatus,
)
...
def mock_registry():
    with patch("app.api.transit._get_registry") as mock:
        ...
        entry.updated_at = datetime(2026, 3, 8, tzinfo=timezone.utc)
...
def test_get_routes(mock_snapshot):
    mock_snapshot.return_value = {
        "routes": [
            TransitRoute(
                route_id="r1", agency_id="a1", short_name="5",
                long_name="Route 5", route_type="3",
                source_provider="test", source_status="live"
            )
        ]
    }
...
def test_get_stops(mock_snapshot):
    mock_snapshot.return_value = {
        "stops": [
            TransitStop(
                stop_id="s1", name="Stop 1", lat=40.4, lon=49.9,
                source_provider="test", source_status="live"
            )
        ]
    }
...
def test_get_network(mock_snapshot):
    mock_snapshot.return_value = {
        "agencies": [1], "routes": [1], "stops": [1, 2],
        "variants": [1], "shapes": [1], "interchanges": [],
        "metadata": {"key": "val"}
    }
...
def test_get_readiness():
    with patch("transit_ingestion.validation.readiness.compute_readiness_report") as mock_report:
        ...
        with patch("transit_ingestion.normalization.merge.build_unified_transit_snapshot"), \
...
def test_get_alerts():
    alert = Alert(
        alert_id="a1", title="Test", published_at=datetime.now(timezone.utc),
        provider="test", affected_mode="bus", alert_category="construction",
        severity="warning", source_provider="test", source_status="live"
    )
    with patch("transit_ingestion.provenance.priority.merge_alerts_by_priority", return_value=[alert]), \
...
def test_get_predicted_arrivals():
    ...
    arrival.predicted_at = datetime.now(timezone.utc)

    status = MagicMock()
    status.stop_id = "s1"
    status.status = "ok"

    with patch("transit_ingestion.providers.yandex_transport_observed.fetch_yandex_stop_page"), \
         patch("transit_ingestion.providers.yandex_transport_observed.normalize_yandex_stop_observations",
...
def test_get_realtime_observations():
    with patch("transit_ingestion.providers.yandex_transport_observed.fetch_yandex_stop_page"), \
         ...
         patch("transit_ingestion.providers.yandex_metro_operational.normalize_yandex_metro_operational",
               return_value=([], [])):
...
def test_get_source_status():
    with patch("transit_ingestion.providers.yandex_traffic_context.get_traffic_context_status", return_value="live"), \
         patch("transit_ingestion.providers.twogis_public_transport.get_twogis_status", return_value="configured"), \
         patch("transit_ingestion.providers.moovit_partner.get_moovit_status", return_value="available"):
...
def test_build_snapshot_logic():
    ...
    with patch("transit_ingestion.providers.bakumetro_official.build_static_metro_network"), \
         ...
         patch("transit_ingestion.providers.bakubus_ayna.normalize.normalize_bus_list") as mock_norm_list, \
         patch("transit_ingestion.normalization.merge.build_unified_transit_snapshot") as mock_merge:

        mock_list.return_value = MagicMock(error=None, data=[{"id": "1"}], fetched_at=datetime.now(timezone.utc))
        mock_id.return_value = MagicMock(error=None, data={"detail": "data"}, fetched_at=datetime.now(timezone.utc))
